...
export function SelectionGroupsModal({
  ...
  async function linkGroupToSelection(groupId: string, groupName: string) {
    ...
    try {
      for (const image of selectedImages) {
        const alreadyLinked = imageGroupIdsByImageId.get(image.fileId)?.has(groupId) ?? false;
        if (alreadyLinked) {
          continue;
        }

        // Ensure the ratings row exists so image_groups FK insert succeeds.
        const ensureRatingResponse = await fetch("/api/ratings", {
          method: "POST",
        ...
        if (!ensureRatingResponse.ok) {
          throw new Error(await getErrorMessage(ensureRatingResponse, "Failed to prepare image-group link"));
        }
      ...
      setNotice(`Linked selected images to \"${groupName}\".`);
    } catch (err) {
  ...
  async function unlinkGroupFromSelection(groupId: string, groupName: string) {
    ...
    try {
      for (const image of selectedImages) {
        const isLinked = imageGroupIdsByImageId.get(image.fileId)?.has(groupId) ?? false;
        if (!isLinked) {
          continue;
        }
        ...
        if (!unlinkResponse.ok && unlinkResponse.status !== 404) {
          throw new Error(await getErrorMessage(unlinkResponse, `Failed to unlink group \"${groupName}\"`));
        }
      }

      await Promise.resolve(onRelationsUpdated());
      setNotice(`Unlinked selected images from \"${groupName}\".`);
    } catch (err) {
